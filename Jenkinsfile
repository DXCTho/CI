  ...
  stages {
   ...
   stage('Artifactory Repo') {
		   ...
		   withCredentials([string(credentialsId: 'JFrog_API_Key', variable: 'TOKEN')]) {
                sh '''
  			curl -H 'X-JFrog-Art-Api:AKCp5cd5L7Ty795tiSVfi4RsT232ZKAvUhQFZQ879je8S5M4M6STd6emAdrqESfVo5dHUPxyZ' -X PUT "http://172.17.0.3:8081/artifactory/libs-release-local/xmp-gitflow-ci-demo-0.1.0-SNAPSHOT.jar" -T target/xmp-gitflow-ci-demo-0.1.0-SNAPSHOT.jar
                '''
            }
